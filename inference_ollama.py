# rag_spark_ollama_strict_logged.py
import os
import time
import json
import pickle
...
spark = (
    SparkSession.builder
        ...
        .config("spark.ui.enabled", "true")           # ← Web UI aktif
        .config("spark.ui.port", "4040")              # ← Port ayarla
        .config("spark.ui.host", "0.0.0.0")          # ← Tüm interface'lere bind
        .config("spark.sql.ui.retainedExecutions", "1000")  # ← Daha fazla execution sakla
        .config("spark.ui.retainedJobs", "1000")     # ← Daha fazla job sakla
        .config("spark.ui.retainedStages", "1000")   # ← Daha fazla stage sakla
...
#    YENİ: (query, answer, ctxs, idxs, dists) döndürüyoruz
# =========================================================
...
for (q, a, ctxs, idxs, dists) in raw:
    rows.append({
        ...
        "context_concat": "\n\n---\n\n".join(ctxs)[:2000]  # CSV için kısaltılmış
    })
df = pd.DataFrame(rows)
csv_path = "logs/retrieval_ollama.csv"
df.to_csv(csv_path, index=False, encoding="utf-8")

# Ekrana da kısa çıktı
